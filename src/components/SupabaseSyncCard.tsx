...
export function SupabaseSyncCard({
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      setSyncStatus({
        tone: 'error',
        message: '서버와 동기화하는 중 문제가 발생했어요. 잠시 후 다시 시도해 주세요.',
      })
    } finally {
      setIsSyncing(false)
    }
  ...
  return (
    <section className="supabase-sync-card panel" aria-label="Supabase 로그인과 동기화">
      <div className="supabase-sync-header">
        <span className="section-label">선택 동기화</span>
        <strong>Supabase 로그인</strong>
        <p>
          로그인하지 않아도 이 앱은 계속 로컬에서 사용할 수 있어요. 로그인하면 지금
          상태를 내 계정의 Supabase에 선택적으로 동기화할 수 있어요.
        </p>
      </div>
      ...
      {isConfigured && user !== null ? (
        <div className="supabase-authenticated">
          ...
          <p className="supabase-help">
            이번 단계에서는 로컬 상태가 기준이며, 서버 동기화는 수동으로만 진행돼요.
          </p>
        </div>
      ) : null}
